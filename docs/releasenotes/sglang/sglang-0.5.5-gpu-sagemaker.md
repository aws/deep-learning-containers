# AWS Deep Learning Containers for SGLang 0.5.5 on SageMaker

AWS Deep Learning Containers for SageMaker are now available with SGLang 0.5.5.

## Announcements

- Introduced SGLang 0.5.5 containers for SageMaker

- Added Python 3.12 support

## Core Packages

| Package | Version |
| --- | --- |
| SGLang | 0.5.5 |
| PyTorch | 2.8.0 |
| TorchVision | 0.23.0 |
| TorchAudio | 2.8.0 |
| Transformers | 4.57.1 |
| CUDA | 12.9.1 |
| cuDNN | 9.16.0.29 |
| NCCL | 2.28.3 |
| EFA | 1.46.0 |

## Security Advisory

AWS recommends that customers monitor critical security updates in the [AWS Security Bulletin](https://aws.amazon.com/security/security-bulletins/).

## Reference

### Docker Image URIs

```

763104351884.dkr.ecr.us-west-2.amazonaws.com/sglang:0.5.5-gpu-py312-cu129-ubuntu22.04-sagemaker

763104351884.dkr.ecr.us-west-2.amazonaws.com/sglang:0.5-gpu-py312-cu129-ubuntu22.04-sagemaker-v1

763104351884.dkr.ecr.us-west-2.amazonaws.com/sglang:0.5.5-gpu-py312

763104351884.dkr.ecr.us-west-2.amazonaws.com/sglang:0.5-gpu-py312

public.ecr.aws/deep-learning-containers/sglang:0.5.5-gpu-py312-cu129-ubuntu22.04-sagemaker

public.ecr.aws/deep-learning-containers/sglang:0.5-gpu-py312-cu129-ubuntu22.04-sagemaker-v1

public.ecr.aws/deep-learning-containers/sglang:0.5.5-gpu-py312

public.ecr.aws/deep-learning-containers/sglang:0.5-gpu-py312
```

### Quick Links

- [Available Images](../../reference/available_images.md)
- [Support Policy](../../reference/support_policy.md)
- [GitHub Repository](https://github.com/aws/deep-learning-containers)
