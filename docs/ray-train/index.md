...
Ray Train shares the `ray` repository with the [Ray Serve DLC](../ray/index.md), using the `train-ml` tag prefix. Pinned tags `train-ml-cuda-v1`,
`train-ml-cuda-v1.0`, and `train-ml-cuda-v1.0.0` are published alongside the floating tag. The image is also available on the
