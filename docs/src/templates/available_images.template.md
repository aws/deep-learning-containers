...
Once you've selected your desired Deep Learning Container image, follow the [Getting Started](../get_started/index.md) guide or explore our [Tutorials](../tutorials/index.md) to begin using it.
...
## {{ repo.display_name }}
{% if repo.public_registry %}

These images are also available in ECR Public Gallery: [{{ repo.name }}](https://gallery.ecr.aws/deep-learning-containers/{{ repo.name }})
{% endif %}

| Framework | Accelerator | Python | CUDA | Platform | Example URL |
| --- | --- | --- | --- | --- | --- |
{% for img in repo.images -%}
| {{ img.framework }} {{ img.version }} | {{ img.accelerator }} | {{ img.python }} | {{ img.cuda }} | {{ img.platform }} | `763104351884.dkr.ecr.<region>.amazonaws.com/{{ repo.name }}:{{ img.tag }}` |
{% endfor %}
