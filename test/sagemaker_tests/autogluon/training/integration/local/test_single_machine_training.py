...
def test_autogluon_local_vision_gpu(
    ...
    config_input = sagemaker_local_session.upload_data(
        path=os.path.join(data_path, "config", "config.vision.gpu.yaml"),
...
def _test_autogluon_local(
    ...
    train_input = sagemaker_local_session.upload_data(
        path=os.path.join(data_path, "training", f"train.{device}.csv"),
        key_prefix=s3_prefix,
    )
    eval_input = sagemaker_local_session.upload_data(
        path=os.path.join(data_path, "evaluation", f"eval.{device}.csv"),
        key_prefix=s3_prefix,
    )
    config_input = sagemaker_local_session.upload_data(
        path=os.path.join(data_path, "config", f"config.{device}.yaml"),
        key_prefix=s3_prefix,
    )
