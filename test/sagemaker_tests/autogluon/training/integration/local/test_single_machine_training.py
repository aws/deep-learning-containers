...
@pytest.mark.model("autogluon")
@pytest.mark.skip_cpu
def test_autogluon_local_vision_object_detection_gpu(docker_image, sagemaker_local_session, instance_type, framework_version, tmpdir):
    ag = AutoGluon(
        entry_point=os.path.join(RESOURCE_PATH, 'scripts', 'train_cv_obj_detection.py'),
        role='SageMakerRole', instance_count=1, instance_type=instance_type,
        sagemaker_session=sagemaker_local_session, image_uri=docker_image,
        framework_version=framework_version, output_path='file://{}'.format(tmpdir),
    )
    ...
    data_path = os.path.join(RESOURCE_PATH, 'data')
    s3_prefix = 'integ-test-data/autogluon'
    config_input = sagemaker_local_session.upload_data(path=os.path.join(data_path, 'config', 'config.vision_obj_detection.gpu.yaml'), key_prefix=s3_prefix)
    ag.fit({'config': config_input})
