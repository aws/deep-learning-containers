...
@pytest.mark.efa()
def test_smmodelparallel_efa(
    ecr_image, sagemaker_regions, instance_type, tmpdir, framework_version, test_script, num_processes
):
...
def _test_smmodelparallel_efa_function(
    ecr_image, sagemaker_session, efa_instance_type, tmpdir, framework_version, test_script, num_processes
):
    """
...
@pytest.mark.efa()
def test_smmodelparallel_multinode_efa(
    ecr_image, sagemaker_regions, instance_type, tmpdir, framework_version, test_script, num_processes
):
    invoke_sm_helper_function(
        ecr_image,
...
@pytest.mark.parametrize(
    ...
    [
        ("tf2_conv.py", 2),
        ("tf2_conv_xla.py", 2),
        ("smmodelparallel_hvd2_conv.py", 4),
        ("send_receive_checkpoint.py", 2),
        ("tf2_checkpoint_test.py", 2),
    ]
)
def test_smmodelparallel(
    ecr_image, sagemaker_regions, instance_type, tmpdir, framework_version, test_script, num_processes
):
    invoke_sm_helper_function(
        ecr_image,
...
def _test_smmodelparallel_function(
    ecr_image, sagemaker_session, instance_type, tmpdir, framework_version, test_script, num_processes
):
    """
    Tests SM Modelparallel in sagemaker
    """
...
@pytest.mark.skip_py2_containers
@pytest.mark.parametrize("test_script, num_processes", [("smmodelparallel_hvd2_conv_multinode.py", 2)])
def test_smmodelparallel_multinode(
    ecr_image, sagemaker_regions, instance_type, tmpdir, framework_version, test_script, num_processes
):
    invoke_sm_helper_function(
        ecr_image,
