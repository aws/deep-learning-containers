...
def make_test_data(
    ...
    for i in range(num_files):
        if num_records > 1:
            build_record_file(
                os.path.join(directory, name + str(i)),
                num_records=num_records,
                dimension=dimension,
            )
        else:
            build_single_record_file(
                os.path.join(directory, name + str(i)), dimension=dimension
            )
...
def run_test(
    ...
    source_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "resources", "pipemode"
    )
