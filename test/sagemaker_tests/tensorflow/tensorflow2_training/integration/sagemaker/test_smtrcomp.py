...
class TestDistributedTraining:
    ...
    def test_native(
        ...
        estimator.fit(
            mnist_dataset, job_name=unique_name_from_base("test-TF-trcomp-DT")
        )
    ...
    def test_parameter_server(
        ...
        estimator.fit(
            mnist_distributed_dataset,
            job_name=unique_name_from_base("test-TF-trcomp-DT-PS"),
        )
...
class TestMLWorkFlow:
    ...
    def test_smdebugger(
        ...
        estimator.fit(
            mnist_dataset, job_name=unique_name_from_base("test-TF-trcomp-debug")
        )
        _assert_model_exported_to_s3(estimator)
        captured = capsys.readouterr()
        _assert_training_compiler_invoked(captured)
    ...
    def test_s3_plugin(
        ...
        estimator.fit(
            mnist_distributed_dataset,
            job_name=unique_name_from_base("test-TF-trcomp-s3"),
        )
        _assert_checkpoints_exported_to_s3(estimator, 10)
        captured = capsys.readouterr()
        _assert_training_compiler_invoked(captured)
    ...
    @pytest.mark.skip(
        reason="SM Training Compiler team yet to implement this integration test"
    )
    ...
    def test_serving(
        ...
        estimator.fit(
            mnist_dataset, job_name=unique_name_from_base("test-TF-trcomp-serving")
        )
        _assert_model_exported_to_s3(estimator)
        captured = capsys.readouterr()
        _assert_training_compiler_invoked(captured)
...
class TestUserExperience:
    ...
    def test_toolkit_fault_attribution(
        ...
        with pytest.raises(Exception):
            smtrcomp_estimator.fit(
                job_name=unique_name_from_base("test-TF-trcomp-fault")
            )
