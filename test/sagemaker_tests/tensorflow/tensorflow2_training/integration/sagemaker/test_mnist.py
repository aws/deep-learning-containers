...
def test_hc_mnist(ecr_image, sagemaker_regions, instance_type, framework_version):
    ...
    invoke_sm_helper_function(
        ecr_image,
        sagemaker_regions,
        _test_mnist_hc_function,
        [training_group],
        framework_version,
    )


def _test_mnist_hc_function(
...
def _test_hc_distributed_mnist_ps_function(
    ...
    estimator.fit(
        inputs, job_name=unique_name_from_base("test-tf-hc-sm-distributed-mnist")
    )
    _assert_checkpoint_exists(
        sagemaker_session.boto_region_name, estimator.model_dir, 0
    )
...
def _test_distributed_mnist_custom_ps(
    ...
    _assert_checkpoint_exists_v2(
        sagemaker_session.boto_region_name, estimator.model_dir, 10
    )
...
def _test_hc_s3_plugin_function(
    ecr_image, sagemaker_session, instance_group, framework_version
):
    resource_path = os.path.join(os.path.dirname(__file__), "..", "..", "resources")
    script = os.path.join(resource_path, "mnist", "mnist_custom.py")
    estimator = TensorFlow(
...
def test_smdebug(ecr_image, sagemaker_regions, instance_type, framework_version):
    invoke_sm_helper_function(
        ecr_image,
        sagemaker_regions,
        _test_smdebug_function,
        instance_type,
        framework_version,
    )


def _test_smdebug_function(
    ecr_image, sagemaker_session, instance_type, framework_version
):
    resource_path = os.path.join(os.path.dirname(__file__), "..", "..", "resources")
...
def _test_smdataparallel_smmodelparallel_mnist_function(
    ...
    if (
        Version(image_framework_version) < Version("2.3.1")
        or image_cuda_version != "cu110"
    ):
        pytest.skip(
