...
parser.add_argument("--size", type=int, default=64, help="Size of tensor to allreduce in MB")
parser.add_argument("--iterations", type=int, default=20, help="Number of times to run allreduce")
parser.add_argument(
    "--num_tensors",
    type=int,
    default=16,
...
parser.add_argument(
    "--warmup", type=int, default=10, help="Number of times to run allreduce and ignore"
)
parser.add_argument(
    "--bucket_size", type=int, default=25, help="Bucket size in MB to perform allreduce as a group"
)
...
def hrg_one_iteration(grads):
    ...
    for i, tensor in enumerate(grads):
        tensor = dist.allreduce(grad=tensor, param_index=i, num_params=len(grads))
...
def test(warmup=False, tensor_size_bytes=104857600, num_tensors=100, iterations=1):
    grad_list = [
        tf.ones(shape=int(tensor_size_bytes / DTSIZE), dtype=DTYPE) for _ in range(num_tensors)
    ]
        ...
        if rank == 0:
            ...
            print(
                ...
                % (
                    ...
                    "%5.3fs" % tdif,
                    tensor_size_bytes,
                    "%5.2fGB/s" % (tensor_size_bytes * num_tensors / 1024 / 1024 / 1024 / tdif),
                ),
                end="\t" if k % 4 != 3 else "\n\t",
            )
            sys.stdout.flush()
            if warmup:
                continue
            bandwidth.append(tensor_size_bytes * num_tensors / 1024 / 1024 / 1024 / tdif)
            artime.append(tdif)
...
if rank == 0:
    ...
    print(
        ...
        % (
            args.info,
            "TF2.SMDATAPARALLEL" if not args.nccl else "TF2.NCCL   ",
            size,
    ...
    if net_mn < THRESHOLD:
        assert False, "The network throughput value {} GB/s is below threshold {} GB/s".format(
            net_mn, THRESHOLD
        )
