...
class LeNet(tf.keras.Model):
    def __init__(self):
        ...
        self.conv1 = Conv2D(
            filters=16, kernel_size=3, padding='valid', 
            strides=(2, 2), input_shape=(None, 28, 28, 1),
            data_format='channels_last', trainable=True,
        )
        
        self.bn1 = BatchNormalization()
        self.conv2 = Conv2D(
            filters= 16, kernel_size=3, strides=(2,2),
...
def load_data(data_dir):
    ...
    """
    x, y = np.load(os.path.join(data_dir, 'train_data.npy')), \
        np.load(os.path.join(data_dir, 'train_labels.npy'))
    
    vx, vy = np.load(os.path.join(data_dir, 'eval_data.npy')), \
        np.load(os.path.join(data_dir, 'eval_labels.npy'))
    ...
    vx, vy = vx.astype(np.float32), vy.astype(int)
    x /= 255.0 
    vx /= 255.0
