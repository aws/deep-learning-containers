...
def generate_jsonlines(shape, payload_size):
    ...
    if payload_size:
        ...
        num_records = _get_num_records_for_json_payload(
            payload_size, len(one_row_string)
        )
...
def generate_data(content_type, shape, payload_size, unit_of_payload="B"):
    ...
    if content_type == _CONTENT_TYPE_JSONLINES:
        ...
        raise ValueError(
            "Content-type {} must be in {}".format(content_type, _VALID_CONTENT_TYPES)
        )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generates test data of different shapes and content types."
    )
    ...
    parser.add_argument(
        "-c",
        "--content-type",
        help="Content type to generate data for.",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "-s",
        "--shape",
        help="Shape of the output data. Behavior depends on content-type. "
    ...
    parser.add_argument(
        "-p",
        "--payload-size",
        help="Approximate payload size. If supplied with text/csv, "
    ...
    parser.add_argument(
        "-u",
        "--unit",
        help='Unit for payload size. One of "B", "KB", or "MB" for bytes, kilobytes, '
