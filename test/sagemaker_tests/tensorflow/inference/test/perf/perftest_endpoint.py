...
class PerfTester(object):
    ...
    def test(self, args, count, test_data):
        ...
        for i in range(args.workers):
            ...
            w = multiprocessing.Process(
                target=self.test_worker, args=(i, args, count, test_data, error_counts)
            )
            workers.append(w)
    ...
    def parse_args(self, args):
        ...
        parser.add_argument(
            "--count", help="number of test iterations", default=1000, type=int
        )
        parser.add_argument(
            "--warmup", help="number of warmup iterations", default=100, type=int
        )
        parser.add_argument(
            "--workers",
            help="number of model workers (and clients)",
            default=1,
            type=int,
        )
...
TEST_DATA = {
    "sm-p2xl": (
        "sagemaker-tensorflow-2018-11-03-14-38-51-707",
        b"[" + _read_file("test/resources/inputs/test-cifar.json") + b"]",
    ),
    "sm-p316xl": (
        "sagemaker-tensorflow-2018-11-03-14-38-51-706",
        b"[" + _read_file("test/resources/inputs/test-cifar.json") + b"]",
    ),
    "tfs-p2xl": (
        "sagemaker-tfs-p2-xlarge",
        _read_file("test/resources/inputs/test-cifar.json"),
    ),
    "tfs-p316xl": (
        "sagemaker-tfs-p3-16xlarge",
        _read_file("test/resources/inputs/test-cifar.json"),
    ),
    "tfs-c5xl": (
        "sagemaker-tfs-c5-xlarge",
        _read_file("test/resources/inputs/test-cifar.json"),
    ),
    "tfs-c518xl": (
        "sagemaker-tfs-c5-18xlarge",
        _read_file("test/resources/inputs/test-cifar.json"),
    ),
    "sm-c5xl": (
        "sagemaker-tensorflow-cifar-c5.xlarge",
        b"[" + _read_file("test/resources/inputs/test-cifar.json") + b"]",
    ),
    "sm-c518xl": (
        "sagemaker-tensorflow-cifar-c5.18xlarge",
        b"[" + _read_file("test/resources/inputs/test-cifar.json") + b"]",
    ),
}
