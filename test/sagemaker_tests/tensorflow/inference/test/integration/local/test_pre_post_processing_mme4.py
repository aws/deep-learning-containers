...
# Only a model-specific inference.py file for the model half_plus_three is provided. Neither a specific inference.py
# file for the model half_plus_two nor a universal inference.py file is provided. It is expected that the handlers
# from the model-specific inference.py file should be used for the model half_plus_three and the default handlers
# should be used for the model half_plus_two.
import json
import os
import subprocess
import sys
...
def container(docker_base_name, tag, runtime_config):
    try:
        command = (
            ...
            " --mount type=volume,source=model_volume_mme4,target=/opt/ml,readonly"
...
def test_unsupported_content_type():
    ...
    for MODEL_NAME in MODEL_NAMES:
        ...
        responses.append(response)
    assert 500 == responses[0].status_code
    assert "unsupported content type" in responses[0].text
    assert 200 == responses[1].status_code
    assert "Invalid value" in responses[1].text
