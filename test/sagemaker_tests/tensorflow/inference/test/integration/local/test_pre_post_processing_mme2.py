...
# The model-specific inference.py files for both models and the universal inference.py file are provided. It is
# expected that the handlers from the model-specific inference.py files should be used and the handlers from the
# universal inference.py file should be ignored.
...
def volume():
    try:
        ...
        subprocess.check_call("docker volume rm model_volume_mme2".split())
...
def container(docker_base_name, tag, runtime_config):
    try:
        command = (
            ...
            " --mount type=volume,source=model_volume_mme2,target=/opt/ml,readonly"
