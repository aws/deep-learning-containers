...
def test_load_two_models():
    ...
    model_data_2 = {"model_name": model_name_2, "url": "/opt/ml/models/half_plus_three"}
...
def test_load_non_existing_model():
    ...
    code, res = make_load_model_request(json.dumps(model_data))
    assert code == 404
    assert "Could not find valid base path {} for servable {}".format(base_path, model_name) in str(
        res
    )
