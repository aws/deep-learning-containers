...
def _test_bucket(region, boto_session):
    ...
    sts_regional_endpoint = "https://sts.{}.amazonaws.com{}".format(
        region, domain_suffix
    )
    sts = boto_session.client(
        "sts", region_name=region, endpoint_url=sts_regional_endpoint
    )
...
def find_or_put_mme_model_data(region, boto_session, mme_folder_name, path_list):
    for local_path in path_list:
        ...
        bucket = _test_bucket(region, boto_session)
        key = (
            "test-tfs/{}/{}".format(mme_folder_name, file)
            if is_model_file
            else "test-tfs/{}/code/{}".format(mme_folder_name, file)
        )
...
def sagemaker_endpoint(
    sagemaker_client, model_name, instance_type, accelerator_type=None
):
    ...
    production_variants = _production_variants(
        model_name, instance_type, accelerator_type
    )
    sagemaker_client.create_endpoint_config(
        EndpointConfigName=model_name, ProductionVariants=production_variants
    )
    ...
    try:
        ...
        status = sagemaker_client.describe_endpoint(EndpointName=model_name)[
            "EndpointStatus"
        ]
        if status != "InService":
...
def _wait_for_transform_job(
    region, boto_session, sagemaker_client, model_name, poll, timeout
):
    status = sagemaker_client.describe_transform_job(TransformJobName=model_name)[
        "TransformJobStatus"
    ]
    job_runtime = 0
    while status == "InProgress":
        ...
        if job_runtime > timeout:
            raise ValueError(
                "Batch transform job {} exceeded maximum runtime {} seconds".format(
                    model_name, timeout
                )
            )
        ...
        if status == "Failed":
            raise ValueError(
                "Failed to execute batch transform job {}".format(model_name)
            )
...
def run_batch_transform_job(
    ...
    with sagemaker_model(
        boto_session, sagemaker_client, image_uri, model_name, model_data
    ):
...
def create_and_invoke_endpoint(
    boto_session,
    sagemaker_client,
    sagemaker_runtime_client,
    model_name,
    ...
    with sagemaker_model(
        ...
        model_name,
        model_data,
        is_multi_model_mode_enabled,
        environment,
    ):
        with sagemaker_endpoint(
            sagemaker_client, model_name, instance_type, accelerator_type
        ):
