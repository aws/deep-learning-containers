...
def pytest_addoption(parser):
    ...
    parser.addoption("--instance-type", default="ml.g5.48xlarge")
    parser.addoption(
...
def pytest_collection_modifyitems(session, config, items):
    ...
    if config.getoption("--generate-coverage-doc"):
        ...
        report_generator.generate_coverage_doc(
            framework="huggingface_tensorflow", job_type="training"
        )
...
def instance_type(request, processor):
    ...
    default_instance_type = "ml.c5.xlarge" if processor == "cpu" else "ml.g5.48xlarge"
...
def skip_p5_tests(request, ecr_image, instance_type):
    if "p5." in instance_type:
        ...
        if image_processor != "gpu" or Version(image_cuda_version.strip("cu")) < Version("120"):
            pytest.skip("Images using less than CUDA 12.0 image doesn't support P5 EC2 instance.")
