...
def pytest_collection_modifyitems(session, config, items):
    if config.getoption("--generate-coverage-doc"):
        ...
        report_generator.generate_coverage_doc(framework="huggingface_tensorflow", job_type="inference")
...
def registry(request, region):
    if request.config.getoption("--aws-id"):
...
def _get_remote_override_flags(region):
    try:
...
def _is_test_disabled(test_name, build_name, version, region):
    """
    ...
    """
    remote_override_flags = _get_remote_override_flags(region)
...
@pytest.fixture(autouse=True)
def disable_test(request, region):
    ...
    if build_name and version and _is_test_disabled(test_name, build_name, version, region):
