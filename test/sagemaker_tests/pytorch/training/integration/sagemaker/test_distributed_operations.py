...
def validate_or_skip_smmodelparallel(ecr_image):
    if not can_run_smmodelparallel(ecr_image):
        pytest.skip(
            "Model Parallelism is supported on CUDA 11 on PyTorch v1.6 and above"
        )
...
def test_smmodelparallel_gpt2_multigpu_singlenode(
    ...
    smp_version = (
        ...
        and Version(framework_version) in SpecifierSet(">=1.11.0")
        else 109
    )
    hyperparameters = {
...
def test_smmodelparallel_gpt2_multigpu_singlenode_flashattn(
    ...
    with timeout(minutes=DEFAULT_TIMEOUT):
        ...
        job_name = utils.unique_name_from_base(
            "test-pt-smdmp-gpt2-singlenode-flashattn"
        )
        invoke_pytorch_estimator(
            ecr_image,
            sagemaker_regions,
...
@pytest.mark.parametrize(
    "test_script, num_processes", [("smmodelparallel_pt_mnist.py", 8)]
)
