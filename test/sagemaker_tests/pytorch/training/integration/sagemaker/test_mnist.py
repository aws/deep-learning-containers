...
@pytest.mark.integration("smexperiments")
@pytest.mark.skip_gpu
def test_mnist_with_native_launcher_distributed_cpu(
    framework_version, ecr_image, sagemaker_regions, instance_type, dist_cpu_backend
):
    instance_type = instance_type or "ml.c4.xlarge"
    function_args = {
    ...
    invoke_pytorch_helper_function(
        ecr_image, sagemaker_regions, _test_mnist_with_native_launcher_distributed, function_args
    )
...
@pytest.mark.integration("smexperiments")
@pytest.mark.skip_cpu
def test_mnist_with_native_launcher_distributed_gpu(
    framework_version, ecr_image, sagemaker_regions, instance_type, dist_gpu_backend
):
    instance_type = instance_type or "ml.p2.xlarge"
    function_args = {
...
def _test_mnist_with_native_launcher_distributed(
    ecr_image, sagemaker_session, framework_version, instance_type, dist_backend, native_launcher
):
    with timeout(minutes=DEFAULT_TIMEOUT):
        pytorch = PyTorch(
            ...
            hyperparameters={
                ...
                "epochs": 1,
                "sagemaker_toolkit_native_launcher_enabled": native_launcher,
            },
        )
        ...
        pytorch.fit(
            ...
            job_name=utils.unique_name_from_base("test-pt-mnist-distributed"),
        )


def _test_mnist_distributed(
    ecr_image, sagemaker_session, framework_version, instance_type, dist_backend
):
    with timeout(minutes=DEFAULT_TIMEOUT):
        pytorch = PyTorch(
            entry_point=mnist_script,
...
def test_hc_mnist_distributed_cpu(
    ...
    invoke_pytorch_helper_function(
        ecr_image, sagemaker_regions, _test_hc_mnist_distributed, function_args
    )
...
def _test_hc_mnist_distributed(
    ecr_image, sagemaker_session, framework_version, instance_groups, dist_backend
):
    with timeout(minutes=DEFAULT_TIMEOUT):
        pytorch = PyTorch(
            entry_point=mnist_script,
        ...
        pytorch.fit(
            {"training": training_input},
            job_name=utils.unique_name_from_base("test-pt-hc-mnist-distributed"),
        )
