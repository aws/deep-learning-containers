...
parser.add_argument("--size", type=int, default=1, help="Size of tensor to allreduce in MB")
parser.add_argument("--iterations", type=int, default=100, help="Number of times to run allreduce")
parser.add_argument(
    "--num_tensors", type=int, default=512, help="How many tensors to allreduce on a single pass"
)
parser.add_argument(
    "--warmup", type=int, default=20, help="Number of times to run allreduce and ignore"
)
parser.add_argument(
    "--bucket_size", type=int, default=25, help="Number of times to run allreduce and ignore"
)
...
if "MASTER_ADDR" not in os.environ:
    ...
    os.environ["MASTER_PORT"] = str(int(os.environ["SMDATAPARALLEL_SERVER_PORT"]) + 2)
...
if args.nccl:
    ...
    dist.init_process_group("smddp", world_size=size, rank=rank, store=None, group_name="")

torch.cuda.set_device(local_rank)

if rank == 0:
    print("NUM WORKERS", size, "WSIZE/RANK/LSIZE/LRANK:", size, rank, local_size, local_rank)
...
def test(warmup=False, size=104857600, num_tensors=100, iterations=1):
    ...
    tests = [torch.ones(int(size / DTSIZE), dtype=DTYPE, device=device) for _ in range(num_tensors)]
    ...
    for k in range(int(iterations)):
        ...
        if rank == 0:
            print(
                ...
                % (
                    ...
                    "%5.3fs" % tdif,
                    size,
                    "%5.2fGB/s" % (size * num_tensors / B_IN_GB / tdif),
                ),
                end="\t" if k % 4 != 3 else "\n\t",
            )
            sys.stdout.flush()
        if warmup:
            continue
        bandwidth.append(size * num_tensors / B_IN_GB / tdif)
        artime.append(tdif)
