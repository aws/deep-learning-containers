...
SCRIPT_PATH = os.path.join(
    RESOURCE_PATH, "default_handlers", "model", "code", "empty_module.py"
)
...
@pytest.mark.skip(
    "Known issue: https://github.com/aws/deep-learning-containers/issues/586"
)
...
def test_batch_transform(
    ...
    invoke_sm_helper_function(
        ecr_image,
        sagemaker_regions,
        _test_batch_transform_function,
        instance_type,
        framework_version,
    )


def _test_batch_transform_function(
    ...
    with timeout.timeout_and_delete_model_with_transformer(
        transformer, sagemaker_session, minutes=20
    ):
    ...
    prediction = _transform_result(
        sagemaker_session.boto_session, transformer.output_path
    )
    assert prediction == 7
