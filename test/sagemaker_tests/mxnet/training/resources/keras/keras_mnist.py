...
def main(batch_size, epochs, num_classes, training_channel, model_dir):
    ...
    if K.image_data_format() == "channels_first":
        ...
        raise ValueError(
            'Unexpected image data format (expected "channels_first"): {}'.format(
                K.image_data_format()
            )
        )
    ...
    data_name, data_shapes = keras.models.save_mxnet_model(
        model=model, prefix=model_prefix, epoch=0
    )

    signature = [
        {"name": data_name[0], "shape": [dim for dim in data_desc.shape]}
