...
def main():
    ...
    def get_mnist_iterator(rank):
        ...
        zip_file_path = download(
            "http://data.mxnet.io/mxnet/data/mnist.zip", dirname=data_dir
        )
    ...
    def conv_nets():
        ...
        with net.name_scope():
            net.add(
                gluon.nn.Conv2D(
                    channels=20, kernel_size=kernel_size, activation=activation
                )
            )
            net.add(gluon.nn.MaxPool2D(pool_size=pool_size, strides=strides))
            net.add(
                gluon.nn.Conv2D(
                    channels=50, kernel_size=kernel_size, activation=activation
                )
            )
            net.add(gluon.nn.MaxPool2D(pool_size=pool_size, strides=strides))
    ...
    for epoch in range(args.epochs):
        ...
        for nbatch, batch in enumerate(train_data, start=1):
            ...
            if nbatch % 100 == 0:
                name, acc = metric.get()
                logging.info(
                    "[Epoch %d Batch %d] Training: %s=%f" % (epoch, nbatch, name, acc)
                )
        ...
        if hvd.rank() == 0:
            ...
            logging.info(
                "Epoch[%d]\tSpeed=%.2f samples/s\tTime cost=%f", epoch, speed, elapsed
            )
