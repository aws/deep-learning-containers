...
def _predictor(image, sagemaker_local_session, instance_type):
    """Context manager for SGLang model deployment and cleanup.

    Model is extracted to /opt/ml/model by SageMaker from model_data tar.gz.
    SGLang loads the model from this local path.
    """
...
def test_sglang_local_chat(docker_image, sagemaker_local_session, instance_type):
    """Test SGLang local deployment with chat completions API."""
    with _predictor(docker_image, sagemaker_local_session, instance_type) as predictor:
        _assert_sglang_chat_prediction(predictor)
