...
def _run_eks_mxnet_multinode_training_horovod_mpijob(
    ...
    random.seed(
        f"{example_image_uri}-{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}"
    )
...
@retry(
    ...
    wait_fixed=12000,
    retry_on_exception=eks_utils.retry_if_value_error,
)
def is_mxnet_eks_multinode_training_complete(job_name, namespace):
    ...
    if (
        ...
        job_condition_succeed = [
            "type" in c and c["type"] == "Succeeded" for c in job_conditions
        ]
