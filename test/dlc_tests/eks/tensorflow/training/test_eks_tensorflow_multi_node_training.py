...
# Test only runs in region us-west-2, on instance type p4d.24xlarge, on PR_EKS_CLUSTER_NAME_TEMPLATE cluster
...
def _run_eks_tensorflow_multinode_training_resnet50_mpijob(
    ...
    script_name = (
        ...
        if major_version == "2"
        else "/deep-learning-models/models/resnet/tensorflow/train_imagenet_resnet_hvd.py"
    )

    args_to_pass = (
        '["--synthetic","--batch_size,128","--num_batches","100","--clear_log","2"]'
        if major_version == "2"
        else '["--num_epochs=1","--synthetic"]'
    )
