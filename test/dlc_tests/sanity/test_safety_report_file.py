...
class SafetyPackageVulnerabilityReport:
    ...
    def __post_init__(self):
        self.vulnerabilities = [
            SafetyVulnerabilityAdvisory(**i) for i in self.vulnerabilities
        ]
...
@pytest.mark.skipif(
    is_canary_context(), reason="Skipping test because it does not run on canary"
)
def test_safety_file_exists_and_is_valid(image):
    ...
    try:
        ...
        safety_file_check = run(
            f"{docker_exec_cmd} test -f {SAFETY_FILE}", warn=True, hide=True
        )
        ...
        safety_report_object = SafetyPythonEnvironmentVulnerabilityReport(
        ...
        assert (
            failed_count == 0
        ), f"{failed_count} package/s failed safety test for {image} !!!"
        LOGGER.info(
...
def test_safety_package_not_installed(image):
    ...
    try:
        ...
        docker_exec_cmd = f"docker exec -i {container_name}"
        safety_package_installed = run(
            f"{docker_exec_cmd} pip show safety", warn=True, hide=True
        )
