...
def _run_instance_role_disabled(image_uri, ec2_client, ec2_instance, ec2_connection):
    ...
    if ecr_utils.get_ecr_image_labels_from_uri(image_uri).get("aws_framework_installed", "true") == "false":
        pytest.skip(f"Image {image_uri} is incompatible with this test")
