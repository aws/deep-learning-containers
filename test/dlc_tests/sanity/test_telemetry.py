...
@pytest.mark.timeout(1200)
def test_telemetry_instance_tag_failure_neuron(
...
def test_telemetry_instance_tag_success_neuron(
    ...
    _run_tag_success_IMDSv2_hop_limit_2(
        neuron, ec2_client, ec2_instance, ec2_connection
    )
...
def test_pytorch_training_job_type_env_var(pytorch_training):
    _, image_framework_version = test_utils.get_framework_and_version_from_tag(
        pytorch_training
    )
...
def _run_tag_failure_IMDSv1_disabled(
    ...
    if expected_tag_key in preexisting_ec2_instance_tags:
        ec2_client.delete_tags(
            Resources=[ec2_instance_id], Tags=[{"Key": expected_tag_key}]
        )
    ...
    LOGGER.info(
        f"_run_tag_failure_IMDSv1_disabled, {image_uri} starting get_ec2_instance_tags"
    )
    ec2_instance_tags = ec2_utils.get_ec2_instance_tags(
        ec2_instance_id, ec2_client=ec2_client
    )
...
def _run_tag_failure_IMDSv2_disabled_as_hop_limit_1(
    ...
    """
    LOGGER.info(
        f"starting _run_tag_failure_IMDSv2_disabled_as_hop_limit_1 with {image_uri}"
    )
    expected_tag_key = "aws-dlc-autogenerated-tag-do-not-delete"
...
def _run_tag_success_IMDSv2_hop_limit_2(
    ...
    LOGGER.info(
        f"_run_tag_success_IMDSv2_hop_limit_2, {image_uri} starting get_ec2_instance_tags"
    )
    preexisting_ec2_instance_tags = ec2_utils.get_ec2_instance_tags(
        ec2_instance_id, ec2_client=ec2_client
    )
...
def invoke_telemetry_call(
    ...
    if "tensorflow" in framework and job_type == "inference":
        ...
        framework_to_import = framework.replace("huggingface_", "").replace(
            "_trcomp", ""
        )
        framework_to_import = (
            "torch" if framework_to_import == "pytorch" else framework_to_import
        )
        ec2_connection.run(
...
def get_tensorflow_inference_command_tf27_above(image_uri, model_name):
    ...
    if Version(image_framework_version) in SpecifierSet(">=2.7"):
        ...
        inference_command = (
            test_utils.build_tensorflow_inference_command_tf27_and_above(
