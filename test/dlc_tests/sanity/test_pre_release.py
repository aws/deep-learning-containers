...
@pytest.mark.model("N/A")
@pytest.mark.canary(
    "Run non-gpu tf serving version test regularly on production images"
)
def test_tf_serving_version_cpu(tensorflow_inference):
    ...
    if re.fullmatch(
        r"(pr-|beta-|nightly-)?tensorflow-inference", image_repo_name
...
def test_framework_version_cpu(image):
    ...
    image_repo_name, _ = get_repository_and_tag_from_image_uri(image)
    if re.fullmatch(
        r"(pr-|beta-|nightly-)?tensorflow-inference(-eia|-graviton)?", image_repo_name
    ):
        pytest.skip(
            ...
            if tested_framework == "torch" and Version(
                ...
                if is_nightly_context():
                    ...
                    if (
                        ...
                        assert (
                            cuda_output == "None"
                        ), f"cuda version has value: {cuda_output}"
                    else:
...
def test_framework_and_neuron_sdk_version(neuron):
    ...
    (
        tested_framework,
        neuron_tag_framework_version,
    ) = get_neuron_framework_and_version_from_tag(image)
...
def test_pip_check(image):
    ...
    if framework == "tensorflow" and Version(framework_version) in SpecifierSet(
        ">=2.9.1"
    ):
        exception_strings = []
...
def test_cuda_paths(gpu):
    ...
    try:
        assert (
            image_tag_in_buildspec
        ), f"Image tag {image_tag} not found in {buildspec_path}"
...
@pytest.mark.skipif(
    ...
    reason="We need to test OSS compliance only on PRs and pipelines",
)
def test_oss_compliance(image):
    ...
    run_cmd_on_container(
        container_name, context, "/usr/local/bin/testOSSCompliance /root"
    )
        ...
        for line in source_code_file:
            ...
            try:
                if os.path.exists(f"{local_file_path}.tar.gz"):
                    ...
                    s3_resource.Object(
                        THIRD_PARTY_SOURCE_CODE_BUCKET, s3_object_path
                    ).load()
                ...
                if e.response["Error"]["Code"] == "404":
                    try:
                        ...
                        object = s3_resource.Bucket(
                            THIRD_PARTY_SOURCE_CODE_BUCKET
