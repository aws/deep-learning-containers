...
def test_repo_anaconda_not_present(image):
    ...
    run_out = test_utils.run_cmd_on_container(container_name, ctx, "conda list --explicit | grep repo.anaconda.com")
    output = run_out.stdout.strip()
    if output:
        raise RuntimeError(f"Image contains packages installed from repo.anaconda.com: {output}")
