...
MX_EC2_EIA_ACCELERATOR_TYPE = get_ec2_accelerator_type(
...
MX_EC2_NEURON_INSTANCE_TYPE = get_ec2_instance_type(
    default="inf1.xlarge", processor="neuron"
)
MX_EC2_GRAVITON_INSTANCE_TYPE = get_ec2_instance_type(
...
@pytest.mark.parametrize(
    "ec2_instance_ami",
    [test_utils.NEURON_UBUNTU_18_BASE_DLAMI_US_WEST_2],
    indirect=True,
)
@pytest.mark.parametrize(
    "ec2_instance_type", MX_EC2_NEURON_INSTANCE_TYPE, indirect=True
)
...
def test_ec2_mxnet_gluonnlp_inference_gpu(
    ...
    run_ec2_mxnet_inference(
        mxnet_inference, BERT_MODEL, "gluonnlp", ec2_connection, "gpu", region, 90, 9091
    )


@pytest.mark.model(SQUEEZENET_MODEL)
@pytest.mark.parametrize("ec2_instance_type", MX_EC2_CPU_INSTANCE_TYPE, indirect=True)
def test_ec2_mxnet_squeezenet_inference_cpu(
    mxnet_inference, ec2_connection, region, cpu_only
):
    run_ec2_mxnet_inference(
        mxnet_inference,
        SQUEEZENET_MODEL,
...
@pytest.mark.parametrize("ec2_instance_type", MX_EC2_CPU_INSTANCE_TYPE, indirect=True)
@pytest.mark.parametrize(
    "ei_accelerator_type", MX_EC2_EIA_ACCELERATOR_TYPE, indirect=True
)
def test_ec2_mxnet_resnet_inference_eia_cpu(
    mxnet_inference_eia, ec2_connection, region
):
...
@pytest.mark.model(RESNET_EIA_MODEL)
@pytest.mark.parametrize(
    "ec2_instance_type", MX_EC2_GPU_EIA_INSTANCE_TYPE, indirect=True
)
@pytest.mark.parametrize(
    "ei_accelerator_type", MX_EC2_EIA_ACCELERATOR_TYPE, indirect=True
)
def test_ec2_mxnet_resnet_inferencei_eia_gpu(
    mxnet_inference_eia, ec2_connection, region
):
    model_name = RESNET_EIA_MODEL
    image_framework, image_framework_version = get_framework_and_version_from_tag(
...
@pytest.mark.model(SQUEEZENET_MODEL)
@pytest.mark.parametrize(
    "ec2_instance_type", MX_EC2_GRAVITON_INSTANCE_TYPE, indirect=True
)
@pytest.mark.parametrize(
...
def test_ec2_mxnet_gluonnlp_inference_cpu(
    ...
    run_ec2_mxnet_inference(
        mxnet_inference, BERT_MODEL, "gluonnlp", ec2_connection, "cpu", region, 90, 9091
    )


def run_ec2_mxnet_inference(
    image_uri,
    model_name,
    container_tag,
    ec2_connection,
    ...
    mms_inference_cmd = test_utils.get_inference_run_command(
...
@pytest.mark.parametrize(
    "ec2_instance_type", MX_EC2_SINGLE_GPU_INSTANCE_TYPE, indirect=True
)
...
def test_mxnet_inference_telemetry_graviton_cpu(
    mxnet_inference_graviton, ec2_connection, cpu_only
):
    execute_ec2_inference_test(
        ec2_connection, mxnet_inference_graviton, MX_TELEMETRY_CMD
    )
