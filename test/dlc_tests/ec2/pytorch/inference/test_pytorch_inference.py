...
PT_EC2_EIA_ACCELERATOR_TYPE = get_ec2_accelerator_type(
    default="eia1.large", processor="eia"
)
PT_EC2_NEURON_INSTANCE_TYPE = get_ec2_instance_type(
...
@pytest.mark.parametrize(
    "ec2_instance_ami", [test_utils.NEURON_INF1_AMI_US_WEST_2], indirect=True
)
@pytest.mark.parametrize(
    "ec2_instance_type", PT_EC2_NEURON_INSTANCE_TYPE, indirect=True
)
...
@pytest.mark.model("densenet")
@pytest.mark.parametrize(
    "ec2_instance_type", PT_EC2_GRAVITON_INSTANCE_TYPE, indirect=True
)
@pytest.mark.parametrize(
...
@pytest.mark.parametrize("ec2_instance_type", PT_EC2_CPU_INSTANCE_TYPE, indirect=True)
@pytest.mark.parametrize(
    "ei_accelerator_type", PT_EC2_EIA_ACCELERATOR_TYPE, indirect=True
)
...
@pytest.mark.model("resnet")
@pytest.mark.parametrize(
    "ec2_instance_type", PT_EC2_GPU_EIA_INSTANCE_TYPE, indirect=True
)
@pytest.mark.parametrize(
    "ei_accelerator_type", PT_EC2_EIA_ACCELERATOR_TYPE, indirect=True
)
...
def test_pytorch_inference_torchdata_gpu(
    ...
    if Version(image_framework_version) in SpecifierSet(">=1.11,<1.14"):
        execute_ec2_inference_test(
            ec2_connection, pytorch_inference, PT_TORCHDATA_DEV_CMD
        )
    else:
...
def test_pytorch_inference_telemetry_graviton_cpu(
    ...
    execute_ec2_inference_test(
        ec2_connection, pytorch_inference_graviton, PT_TELEMETRY_CMD
    )
