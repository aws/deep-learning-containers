...
def test_pytorch_2_2_gpu(pytorch_training___2__2, ec2_connection, gpu_only, ec2_instance_type):
    ...
    with ThreadPoolExecutor() as ex:
        task_handler = {ex.submit(fn[0], *fn[1]): fn for fn in thread_tasks}

        for h in as_completed(task_handler):
            task = task_handler[h]
            try:
                resp = h.result()
            except Exception as e:
                exceptions.append(f"{task[0].__name__} FAILED WITH {type(e).__name__}:\n{e}")

    assert not exceptions, "\n\n\n".join(exceptions)


# @pytest.mark.usefixtures("sagemaker")
# @pytest.mark.integration("all PT 2.0 tests")
# @pytest.mark.model("N/A")
# @pytest.mark.team("conda")
# @pytest.mark.parametrize("ec2_instance_type", PT_EC2_GPU_INSTANCE_TYPE, indirect=True)
# def test_pytorch_2_0_gpu(pytorch_training___2__0, ec2_connection, gpu_only, ec2_instance_type):
#     pytorch_training = pytorch_training___2__0
#     if test_utils.is_image_incompatible_with_instance_type(pytorch_training, ec2_instance_type):
#         pytest.skip(
#             f"Image {pytorch_training} is incompatible with instance type {ec2_instance_type}"
#         )
...
#     if "cu118" in pytorch_training:
#         _pytorch_healthcheck_dcgm(pytorch_training, ec2_connection)
#         _pytorch_healthcheck_nccl(pytorch_training, ec2_connection)
#         _pytorch_train_dgl_gpu(pytorch_training, ec2_connection)
...
def _pytorch_healthcheck_dcgm(pytorch_training, ec2_connection):
    execute_ec2_training_test(
        ec2_connection, pytorch_training, PT_DCGM_TEST_CMD, container_name="pt_dcgm"
    )


def _pytorch_healthcheck_nccl(pytorch_training, ec2_connection):
    execute_ec2_training_test(
        ec2_connection,
        pytorch_training,
        PT_NCCL_LOCAL_TEST_CMD,
        container_name="pt_healthcheck_nccl",
    )
...
def _pytorch_train_dgl_gpu(pytorch_training, ec2_connection):
    execute_ec2_training_test(ec2_connection, pytorch_training, PT_DGL_CMD, container_name="dgl")
