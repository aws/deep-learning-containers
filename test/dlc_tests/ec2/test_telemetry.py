...
def _run_s3_query_bucket_success(image_uri, ec2_client, ec2_instance, ec2_connection):
    ...
    # cause we are using a different telemetry script for them
    # if ((framework == "pytorch" and Version(framework_version) <= Version("2.6")) or (framework == "tensorflow" and Version(framework_version) <= Version("2.18"))):
    #     expected_s3_url = (
    #         "https://aws-deep-learning-containers-{0}-beta-878799496459.s3.{0}.amazonaws.com"
    ...
    expected_s3_url = (
        ...
        "/dlc-containers-{2}.txt?x-instance-id={2}&x-framework={3}&x-framework_version={4}&x-py_version={5}".format(
            hashed_region, image_region, ec2_instance_id, framework, framework_version, py_version
        )
    )
