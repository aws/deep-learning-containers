...
TF_EC2_EIA_ACCELERATOR_TYPE = get_ec2_accelerator_type(
    default="eia1.large", processor="eia"
)
TF_EC2_NEURON_ACCELERATOR_TYPE = get_ec2_instance_type(
...
@pytest.mark.parametrize(
    "ec2_instance_type", TF_EC2_NEURON_ACCELERATOR_TYPE, indirect=True
)
@pytest.mark.parametrize(
    "ec2_instance_ami", [test_utils.UL20_TF_NEURON_US_WEST_2], indirect=True
)
def test_ec2_tensorflow_inference_neuron(
    tensorflow_inference_neuron, ec2_connection, region
):
    run_ec2_tensorflow_inference(
        tensorflow_inference_neuron, ec2_connection, "8500", region
    )
...
def test_ec2_tensorflow_inference_cpu(
...
@pytest.mark.parametrize("ec2_instance_type", TF_EC2_CPU_INSTANCE_TYPE, indirect=True)
@pytest.mark.parametrize(
    "ei_accelerator_type", TF_EC2_EIA_ACCELERATOR_TYPE, indirect=True
)
def test_ec2_tensorflow_inference_eia_cpu(
    tensorflow_inference_eia, ec2_connection, region
):
    run_ec2_tensorflow_inference(
        tensorflow_inference_eia, ec2_connection, "8500", region
    )
...
@pytest.mark.model("mnist")
@pytest.mark.parametrize(
    "ec2_instance_type", TF_EC2_SINGLE_GPU_INSTANCE_TYPE, indirect=True
)
def test_ec2_tensorflow_inference_gpu_telemetry(
    ...
    run_ec2_tensorflow_inference(
        tensorflow_inference, ec2_connection, "8500", region, True
    )
...
@pytest.mark.model("mnist")
@pytest.mark.parametrize(
    "ec2_instance_type", TF_EC2_GRAVITON_INSTANCE_TYPE, indirect=True
)
@pytest.mark.parametrize(
...
def test_ec2_tensorflow_inference_graviton_cpu(
    ...
    run_ec2_tensorflow_inference(
        tensorflow_inference_graviton, ec2_connection, "8500", region
    )
...
def run_ec2_tensorflow_inference(
    ...
    try:
        ...
        remote_out = ec2_connection.run(
            f"docker logs {container_name}", warn=True, hide=True
        )
        LOGGER.info(
            f"--- TF container logs ---\n--- STDOUT ---\n{remote_out.stdout}\n--- STDERR ---\n{remote_out.stderr}"
        )
...
def train_mnist_model(serving_folder_path, ec2_connection):
    ...
    mnist_script_path = (
        f"{serving_folder_path}/tensorflow_serving/example/mnist_saved_model.py"
    )
    ec2_connection.run(
...
def host_setup_for_tensorflow_inference(
    ...
    ec2_connection.run(
        (f"sudo apt-get update && sudo apt-get install -y python3-pip"), hide=True
    )
    ...
    if is_graviton:
        ec2_connection.run(
            f"pip install --no-cache-dir -U tensorflow-cpu-aws", hide=True
        )
        ec2_connection.run(
            (
    ...
    if str(framework_version).startswith(TENSORFLOW1_VERSION):
        ...
        if is_neuron:
            ...
            neuron_model_file_path = os.path.join(
                serving_folder_path, f"models/{model_name}/1"
            )
        ...
        local_scripts_path = os.path.join(
            "container_tests", "bin", "tensorflow_serving"
        )
        ...
        if is_neuron:
            ...
            neuron_model_file_path = os.path.join(
                serving_folder_path, "models", "model_name", "1"
            )
            LOGGER.info(f"Host Model path {neuron_model_file_path}")
...
def check_telemetry(ec2_connection, container_name):
    ec2_connection.run(
        f"docker exec -i {container_name} bash -c '[ -f /tmp/test_request.txt ]'"
    )
    ec2_connection.run(
