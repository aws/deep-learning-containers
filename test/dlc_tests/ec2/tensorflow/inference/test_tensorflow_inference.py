...
def host_setup_for_tensorflow_inference(
    ...
    if is_graviton:
        ...
        ec2_connection.run((f"{python_invoker} -m pip3 install --no-cache-dir -U {TF_URL}"), hide=True)
        ec2_connection.run(
            (
                f"{python_invoker} -m pip3 install --no-dependencies --no-cache-dir tensorflow-serving-api=={framework_version}"
            ),
            hide=True,
        )
    else:
        ec2_connection.run(
            (
                f"{python_invoker} -m pip3 install --user -qq -U 'tensorflow<={framework_version}' "
    ...
    if str(framework_version).startswith(TENSORFLOW1_VERSION):
        run_out = ec2_connection.run(f"git clone https://github.com/tensorflow/serving.git {serving_folder_path}")
