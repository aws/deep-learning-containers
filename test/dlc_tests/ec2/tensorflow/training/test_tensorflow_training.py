...
TF_EC2_HPU_INSTANCE_TYPE = get_ec2_instance_type(
...
def test_tensorflow_with_horovod_gpu(
    ...
    test_script = (
        TF1_HVD_CMD if is_tf_version("1", tensorflow_training) else TF2_HVD_CMD
    )
...
def test_tensorflow_with_horovod_cpu(
    ...
    try:
        ...
        if "TF HVD tests passed!" in debug_stdout:
            ...
            return
        raise TFTrainingTestFailure(
            f"TF HVD test failed. Full output:\n{debug_stdout}"
        ) from e
...
def test_tensorflow_telemetry_gpu(
    tensorflow_training, ec2_connection, gpu_only, ec2_instance_type
):
    if test_utils.is_image_incompatible_with_instance_type(
        tensorflow_training, ec2_instance_type
...
def test_tensorflow_keras_horovod_fp32(
    ...
    execute_ec2_training_test(
        ec2_connection, tensorflow_training, TF_KERAS_HVD_CMD_FP32
    )
...
@pytest.mark.model("sequential")
@pytest.mark.parametrize("ec2_instance_type", TF_EC2_CPU_INSTANCE_TYPE, indirect=True)
def test_tensorflow_tensorboard_cpu(
    tensorflow_training, ec2_connection, tf2_only, cpu_only
):
    execute_ec2_training_test(ec2_connection, tensorflow_training, TF_TENSORBOARD_CMD)
...
def run_data_service_test(ec2_connection, tensorflow_training, cmd):
    _, tensorflow_version = test_utils.get_framework_and_version_from_tag(
        tensorflow_training
    )
    ...
    execute_ec2_training_test(
        ec2_connection, tensorflow_training, cmd, host_network=True
    )
