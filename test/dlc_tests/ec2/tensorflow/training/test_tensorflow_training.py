...
def run_data_service_test(ec2_connection, tensorflow_training, cmd):
    ec2_connection.run('/usr/bin/python3.7 -m pip install --upgrade pip')
    ec2_connection.run('pip3 install tensorflow==2.4')
    container_test_local_dir = os.path.join("$HOME", "container_tests")
    ec2_connection.run(f'cd {container_test_local_dir}/bin && screen -d -m /usr/bin/python3.7 start_dataservice.py')
