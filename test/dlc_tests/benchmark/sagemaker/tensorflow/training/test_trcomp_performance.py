...
def pytest_generate_tests(metafunc):
    if "instance_type" in metafunc.fixturenames:
        metafunc.parametrize(
            ...
            [
                pytest.param(
                    "ml.p3.2xlarge", 1, marks=[pytest.mark.p3, pytest.mark.single_gpu]
                ),
...
class TestImageClassification:
    ...
    def test_resnet101_at_fp16(
        ...
        for batch in np.array(batches, dtype=int):
            ...
            threshold = TRCOMP_THRESHOLD["tensorflow"][short_version]["resnet101"][
                instance_type
            ][instance_count][batch]
            result = (
...
class TestNLPModels:
    ...
    def test_gpt2(
        ...
        short_version = ".".join(framework_version.split(".")[:2])
        threshold = TRCOMP_THRESHOLD["tensorflow"][short_version]["GPT-2"][
            instance_type
        ][instance_count][batch]
        result = (
