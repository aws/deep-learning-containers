...
def test_performance_pytorch_rn50_hpu_synthetic(pytorch_training_habana, ec2_connection, cards_num):
    ...
    execute_ec2_training_performance_test(
        ...
        post_process=post_process_pytorch_rn50_hpu_py3_synthetic_ec2_training_performance,
        data_source="synthetic",
        threshold={"Throughput": threshold},
        cards_num=cards_num,
...
def test_performance_pytorch_bert_hpu(pytorch_training_habana, ec2_connection, cards_num):
    ...
    execute_ec2_training_performance_test(
        ...
        post_process=post_process_pytorch_bert_hpu_py3_synthetic_ec2_training_performance,
        data_source="synthetic",
        threshold={"Throughput": threshold},
        cards_num=cards_num,
        synapseai_version=synapseai_version
...
def post_process_pytorch_rn50_hpu_py3_synthetic_ec2_training_performance(connection, log_location):
    ...
    for line in lines:
        if "img/s:" in line:
            perf_line = re.compile(r"] .+ img/s: (\d+\.\d+)")
            res_line = perf_line.search(line)
            if res_line:
                #Skip first 10 iterations for perf
                if iteration_num < 10:
                    iteration_num += 1
                else:
                    eps[step_num] = list()
...
def post_process_pytorch_bert_hpu_py3_synthetic_ec2_training_performance(connection, log_location):
    lines = connection.run(f"cat {log_location}").stdout.split("\n")
    throughput = 0.0
    results = dict()
    perf_line = re.compile(r"(.*),  (\d+\.\d+)(\S+\/\S+)]")
    line_before_last = False
    for line in reversed(lines):
        if "Iteration" in line:
            if not line_before_last:
                line_before_last = True
                continue
            obj = perf_line.search(line)
            if obj:
                if obj.group(3) == "s/it":
                    itps = 1.0 / float((obj.group(2)))
                else:
                    itps = float((obj.group(2)))
                throughput = itps * 24.0
                break
    return {"Throughput": throughput}
