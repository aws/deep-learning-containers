...
@pytest.mark.parametrize(
    "ec2_instance_ami", [UBUNTU_18_BASE_DLAMI_US_WEST_2], indirect=True
)
@pytest.mark.parametrize(
    "ec2_instance_type", PT_EC2_GPU_INDUCTOR_INSTANCE_TYPES, indirect=True
)
def test_performance_pytorch_gpu_inductor_torchbench(
...
def test_performance_pytorch_gpu_inductor_timm_models(
    pytorch_training, ec2_connection, gpu_only, py3_only, ec2_instance_type
):
    _, image_framework_version = get_framework_and_version_from_tag(pytorch_training)
    if Version(image_framework_version) < Version("2.0"):
...
def test_performance_pytorch_gpu_inductor_huggingface(
    pytorch_training, ec2_connection, gpu_only, py3_only, ec2_instance_type
):
    _, image_framework_version = get_framework_and_version_from_tag(pytorch_training)
    if Version(image_framework_version) < Version("2.0"):
...
def execute_ec2_training_performance_test(
    ...
    s3_key = os.path.join(
        os.sep, model_suite, re.sub("\.", "-", ec2_instance_type), timestamp
    )
    s3_pth = BENCHMARK_RESULTS_S3_BUCKET_TRCOMP + s3_key
    test_prefix = os.path.join(
        CONTAINER_TESTS_PREFIX,
        "benchmark",
        f"run_pytorch_inductor_training_benchmark_gpu_{model_suite}",
    )
    ...
    connection.run(
        f"{docker_cmd} pull {ecr_uri}", hide="out"
    )  # this line fixes the time out issue
    ...
    try:
        subprocess.check_output(
            f"aws s3 cp {s3_pth}/ {model_suite}/ --recursive", shell=True
        )
        read_upload_benchmarking_result_to_cw(
            METRIC_NAMES,
            model_suite,
...
def read_upload_benchmarking_result_to_cw(
    metric_names,
    pth,
    instance_type="p4d.24xlarge",
    model_suite="huggingface",
    precision="amp",
    namespace="PyTorch/EC2/Benchmarks/TorchDynamo/Inductor",
):
    dimensions = [
