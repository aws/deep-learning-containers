...
PT_MNIST_TRAINING_SCRIPT = os.path.join(
    CONTAINER_TESTS_PREFIX, "pytorch_tests", "testPyTorch"
)
PT_DGL_TRAINING_SCRIPT = os.path.join(
    CONTAINER_TESTS_PREFIX, "dgl_tests", "testPyTorchDGL"
)
...
PT_MLP_NEURON_TRAINING_SCRIPT = (
    f"torchrun --nproc_per_node=2 --nnodes=1 "
    + os.path.join(CONTAINER_TESTS_PREFIX, "pytorch_tests", "testNeuronMlp")
)
...
@pytest.mark.parametrize(
    "training_script", [PT_MLP_NEURON_TRAINING_SCRIPT], indirect=True
)
