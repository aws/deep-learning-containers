...
def test_ecs_pytorch_inference_cpu(
    pytorch_inference, ecs_container_instance, region, cpu_only
):
...
def test_ecs_pytorch_inference_graviton_cpu(
    ...
    __ecs_pytorch_inference_cpu(
        pytorch_inference_graviton, ecs_container_instance, region
    )
...
def __ecs_pytorch_inference_cpu(pytorch_inference, ecs_container_instance, region):
    ...
    try:
        ...
        assert (
            inference_result
        ), f"Failed to perform inference at IP address: {public_ip_address}"
...
def test_ecs_pytorch_inference_neuron(
    pytorch_inference_neuron, ecs_container_instance, region
):
    worker_instance_id, ecs_cluster_arn = ecs_container_instance
    public_ip_address = ec2_utils.get_public_ip(worker_instance_id, region=region)
    num_neurons = ec2_utils.get_instance_num_inferentias(
        worker_instance_id, region=region
    )
...
def test_ecs_pytorch_inference_gpu(
    pytorch_inference, ecs_container_instance, region, gpu_only
):
    worker_instance_id, ecs_cluster_arn = ecs_container_instance
    public_ip_address = ec2_utils.get_public_ip(worker_instance_id, region=region)
