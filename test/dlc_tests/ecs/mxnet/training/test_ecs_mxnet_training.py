...
@pytest.mark.parametrize("ecs_ami", [ECS_AML2_CPU_USWEST2], indirect=True)
def test_ecs_mxnet_training_dgl_cpu(cpu_only, ecs_container_instance, mxnet_training, training_cmd, ecs_cluster_name):
    """
    ...
    the task to be stopped before doing teardown operations of instance and cluster.
    """
    if "py2" in mxnet_training:
        pytest.skip(msg=f"DGL is not supported on Python 2. Skipping container {mxnet_training} with py2 in tag.")
