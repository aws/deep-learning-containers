...
@pytest.mark.parametrize("training_script", [MX_GLUONNLP_TRAINING_SCRIPT], indirect=True)
...
def test_ecs_mxnet_training_gluonnlp_cpu(cpu_only, py3_only, ecs_container_instance, mxnet_training, training_cmd,
                                         ecs_cluster_name):
    """
    CPU Gluon NLP test for MXNet Training

    Instance Type - c4.xlarge

    Gluon NLP is only supported in py3, hence we have used the "py3_only" fixture to ensure py2 images don't run
    on this function.
...
@pytest.mark.parametrize("training_script", [MX_GLUONNLP_TRAINING_SCRIPT], indirect=True)
@pytest.mark.parametrize("ecs_instance_type", ["g3s.xlarge"], indirect=True)
@pytest.mark.parametrize("ecs_ami", [ECS_AML2_GPU_USWEST2], indirect=True)
