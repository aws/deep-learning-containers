...
def lookup_condition(lookup, image):
    ...
    print(f"[logs-ABCD][Displaying] repo_name: {repo_name} lookup: {lookup} image: {image}")
...
def pytest_generate_tests(metafunc):
    ...
    for fixture in FRAMEWORK_FIXTURES:
        if fixture in metafunc.fixturenames:
            ...
            for image in images:
                print(f"[logs-ABCD][Checking] image:{image} lookup:{lookup}")
                if lookup_condition(lookup, image):
                    print(f"[logs-ABCD][True condition] image:{image} lookup:{lookup}")
                    is_example_lookup = (
