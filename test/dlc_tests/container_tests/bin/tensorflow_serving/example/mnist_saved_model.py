...
def main(_):
    ...
    tf_example = tf.io.parse_example(
        serialized=serialized_tf_example, features=feature_configs
    )
    ...
    keys_tensor = tf.constant(list(range(10)))
    vals_tensor = tf.constant([str(i) for i in range(10)])
    table = tf.lookup.StaticHashTable(
        tf.lookup.KeyValueTensorInitializer(keys_tensor, vals_tensor), "-1"
    )

    prediction_classes = table.lookup(tf.cast(indices, dtype=tf.int32))
    for _ in range(FLAGS.training_iteration):
        batch = mnist.train.next_batch(50)
        train_step.run(feed_dict={x: batch[0], y_: batch[1]})
    correct_prediction = tf.equal(
        tf.argmax(input=y, axis=1), tf.argmax(input=y_, axis=1)
    )
