...
class TensorflowModelServerTester(object):
    ...
    def _Predict(
        self, model_server_address, request_timeout=30, iterations=NUM_PREDICTIONS
    ):
        ...
        for ii in range(len(input_names)):  # Goverened by the input_names'
            ...
            if input_types[ii] != None:
                ...
                request.inputs[input_names[ii]].CopyFrom(
                    tf.compat.v1.make_tensor_proto(
                        input_data[ii], shape=input_shapes[ii]
                    )
                )
...
def test_serving(
    ServingTests, binary=None, concurrent=1, exclude_list=[], iterations=2
):
    results = {}
...
def main():
    ...
    parser.add_argument(
        "--upload_all",
        help="Upload all models in model_info definition",
        action="store_true",
    )
    parser.add_argument(
        "--binary",
        default="/usr/bin/tensorflow_model_server",
        help="Where tensorflow_model_server",
    )
    parser.add_argument(
        "--concurrent",
        help="Number of concurrent threads (GIL limited)",
        default=1,
        type=int,
    )
    parser.add_argument(
    ...
    if args.iterations:
        ...
        iterations = 2
    server_results = {}
    server_results = test_serving(
        ServingTests, args.binary, args.concurrent, iterations=iterations
    )
    if args.get_perf:
