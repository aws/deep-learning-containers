...
def test_nccl_version():
    try:
        ...
        result = subprocess.Popen(
            ["sed", "-r", "s/^.*\.so\.//"], stdin=tail.stdout, stdout=PIPE
        )
        output, error = result.communicate()
        system_nccl = [int(x, 10) for x in output.decode("utf-8").strip().split(".")]
        assert len(system_nccl) == 3, "Error: failed to parse system nccl version"
        ...
        assert (
            pt_nccl == system_nccl
        ), "Error: PT NCCL version does not match system NCCL: {} vs {}".format(
            pt_nccl, system_nccl
        )
