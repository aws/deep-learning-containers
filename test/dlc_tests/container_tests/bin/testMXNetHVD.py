...
def parse_args():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=8, help="training batch size per device (CPU/GPU)."
    )
    parser.add_argument(
        "-j",
        "--num-data-workers",
...
def get_synthetic_dataloader(batch_size, num_workers):
    ...
    data = mx.nd.array(np.random.uniform(-1, 1, data_shape), dtype=np.float32)

    dataset = mx.gluon.data.dataset.ArrayDataset(data, label)
    train_data = mx.gluon.data.DataLoader(
        dataset, batch_size=batch_size, num_workers=num_workers, shuffle=True, last_batch="discard"
    )
    return train_data
