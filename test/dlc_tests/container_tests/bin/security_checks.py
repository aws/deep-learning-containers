...
def main():
    ...
    check_if_any_files_in_subfolder_with_mask_was_last_modified_before_the_boottime(
        home_dir, "history", recursive=False
    )
    check_if_any_files_in_subfolder_with_mask_was_last_modified_before_the_boottime(
...
def check_that_cache_dir_is_removed(home_dir):
    ...
    if os.path.exists(cache_dir_path):
        ...
        is_autogluon: bool = (
            os.popen("pip freeze | grep autogluon | wc -l").read().replace("\n", "")
            != "0"
        )
        if is_autogluon and (
            sorted(content_of_cache_dir) == ["mim", "pip"]
            or content_of_cache_dir == ["pip"]
        ):
            ...
            if not sorted(content_of_cache_dir)[0].startswith("pip"):
                raise ValueError(
                    "cache dir include file that it probably should not have: {}".format(
                        content_of_cache_dir[0]
                    )
                )
...
def check_that_global_tmp_dir_is_empty():
    ...
    for f in global_tmp_dir_content:
        if (
            not f.startswith(".")
            and "system" not in f.lower()
            and "dkms" not in f.lower()
            and "ccNPSUr9.s" not in f
            and "hsperfdata" not in f
        ):
            raise ValueError(
                "/tmp folder includes file that probably should not be there: {}".format(
                    f
                )
            )
...
def check_if_any_files_in_subfolder_with_mask_was_last_modified_before_the_boottime(
    folder, mask=None, recursive=True
):
    uptime_seconds = 0
    if recursive:
    ...
    for f in all_files:
        ...
        if last_modified_time_seconds < boot_time_seconds:
            raise ValueError(
                "Looks like {} was modified before the current boot".format(f)
            )
