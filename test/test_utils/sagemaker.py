...
def generate_sagemaker_pytest_cmd(image, sagemaker_test_type):
    ...
    local_test_report = os.path.join(
        UBUNTU_HOME_DIR, "test", f"{job_type}_{tag}_sm_local.xml"
    )
    ...
    if framework == "tensorflow" and job_type == "training":
        path = os.path.join(
            os.path.dirname(path), f"{framework}{framework_major_version}_training"
        )
...
def execute_local_tests(image, pytest_cache_params):
    ...
    ec2_test_report_path = os.path.join(
        UBUNTU_HOME_DIR, "test", f"{job_type}_{tag}_sm_local.xml"
    )
    instance_id = ""
    ec2_conn = None
...
def execute_sagemaker_remote_tests(
    process_index, image, global_pytest_cache, pytest_cache_params
):
    """
    ...
    with context.cd(path):
        ...
        with context.prefix(f"source {tag}/bin/activate"):
            ...
            pytest_command += (
                f" -o cache_dir={os.path.join(str(process_index), '.pytest_cache')}"
            )
