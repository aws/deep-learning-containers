...
def assign_sagemaker_local_job_instance_type(image):
    if "graviton" in image:
        ...
        return "g5.8xlarge"
    return "p5.48xlarge" if "gpu" in image else "c5.18xlarge"
...
def launch_sagemaker_local_ec2_instance(image, ec2_key_name, region):
    ...
    instance = ec2_utils.launch_instance(
        ...
        instance_name=f"sallyseo-sm-local-{instance_name}", ### sallyseo
    )
...
def execute_local_tests(image, pytest_cache_params):
    ...
    ec2_key_name = f"sallyseo_{job_type}_{tag}_sagemaker_{random.randint(1, 1000)}" ### sallyseo
    region = os.getenv("AWS_REGION", DEFAULT_REGION)
