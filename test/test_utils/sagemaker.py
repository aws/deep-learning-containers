...
def assign_sagemaker_remote_job_instance_type(image):
    if "graviton" in image or "arm64" in image:
        ...
        all(word in image for word in ["pytorch", "inference", "gpu"])
        and "huggingface" not in image
    ):
        return "ml.g5.8xlarge"
    elif "gpu" in image:
        if "pytorch" in image and "training" in image and "huggingface" not in image:
...
def assign_sagemaker_local_job_instance_type(image):
    if "graviton" in image or "arm64" in image:
        ...
        return "g5.8xlarge"
    elif all(word in image for word in ["huggingface-vllm-omni", "gpu"]):
        return "g5.xlarge"
                    ...
                    if py_version == "py310"
                        ...
                        if py_version == "py311"
                            ...
                            if py_version == "py312"
                                ...
                                "313"
                                if py_version == "py313"
                                else "2" if py_version == "py27" else "3"
                            )
                        )
                    )
                )
