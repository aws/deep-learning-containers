...
class AllowListFormatVulnerabilityForEnhancedScan:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.package_details = (
            None if packageVulnerabilityDetails else VulnerablePackageDetails(**kwargs["package_details"])
        )
        self.remediation = remediation
        self.source_url = (
            packageVulnerabilityDetails["sourceUrl"] if packageVulnerabilityDetails else kwargs["source_url"]
        )
        ...
        self.cvss_v3_severity = (
            self.get_cvss_v3_severity(self.cvss_v3_score) if packageVulnerabilityDetails else kwargs["cvss_v3_severity"]
        )
    ...
    def __eq__(self, other):
        ...
        ## Also ignore the description key of the vulnerability, because description may be changed for the same vulnerability. This fails during comparison.
        if test_utils.check_if_two_dictionaries_are_equal(
            ...
            return test_utils.check_if_two_dictionaries_are_equal(
                dataclasses.asdict(self), dataclasses.asdict(other),
                ignore_keys=["package_details", "title", "description"]
            )
        return False
    ...
    def get_sorted_vulnerability_list(self):
        ...
        for key, list_of_complex_types in copy_dict.items():
            ...
            uniquified_list.sort(
                key=lambda dict_element: dict_element["name"] if isinstance(dict_element, dict) else dict_element.name
            )
...
class ECREnhancedScanVulnerabilityList(ScanVulnerabilityList):
    ...
    def construct_allowlist_from_allowlist_formatted_vulnerabilities(
        self, allowlist_formatted_vulnerability_list: List[AllowListFormatVulnerabilityForEnhancedScan]
    ):
        """
