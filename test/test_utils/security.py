...
class VulnerablePackageDetails:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.name = name
        self.package_manager = kwargs.get("packageManager") or kwargs.get(
            "package_manager"
        )
...
class AllowListFormatVulnerabilityForEnhancedScan:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.package_name = (
            None if packageVulnerabilityDetails else kwargs["package_name"]
        )
        ...
        self.cvss_v3_score = (
            self.cvss_v31_score if self.cvss_v31_score > 0 else self.cvss_v30_score
        )
        self.cvss_v2_score = (
    ...
    def __eq__(self, other):
        assert type(self) == type(
            other
        ), f"Types {type(self)} and {type(other)} mismatch!!"
    ...
    def get_cvss_score(
        self, packageVulnerabilityDetails: dict, score_version: str = "3.1"
    ):
        """
...
class ScanVulnerabilityList:
    ...
    def get_sorted_vulnerability_list(self):
        ...
        for key, list_of_complex_types in copy_dict.items():
            uniquified_list = test_utils.uniquify_list_of_complex_datatypes(
                list_of_complex_types
            )
    ...
    def __contains__(self, vulnerability):
        ...
        for allowed_vulnerability in self.vulnerability_list[package_name]:
            if self.are_vulnerabilities_equivalent(
                vulnerability, allowed_vulnerability
            ):
                return True
        return False
    ...
    def __cmp__(self, other):
        ...
        if sorted(self.vulnerability_list.keys()) != sorted(
            other.vulnerability_list.keys()
        ):
            return False
    ...
    def __add__(self, other):
        ...
        all_vulnerabilities = (
            flattened_vulnerability_list_self + flattened_vulnerability_list_other
        )
        if not all_vulnerabilities:
            return None
        union_vulnerabilities = test_utils.uniquify_list_of_complex_datatypes(
            all_vulnerabilities
        )

        union = type(self)(minimum_severity=self.minimum_severity)
        union.construct_allowlist_from_allowlist_formatted_vulnerabilities(
            union_vulnerabilities
        )
        return union
...
class ECREnhancedScanVulnerabilityList(ScanVulnerabilityList):
    ...
    def construct_allowlist_from_allowlist_formatted_vulnerabilities(
        self,
        allowlist_formatted_vulnerability_list: List[
    ...
    def construct_allowlist_from_ecr_scan_result(self, ecr_format_vulnerability_list):
        ...
        for ecr_format_vulnerability in ecr_format_vulnerability_list:
            for vulnerable_package in ecr_format_vulnerability[
                "packageVulnerabilityDetails"
            ]["vulnerablePackages"]:
                ...
                vulnerable_package_object = VulnerablePackageDetails(
                    **vulnerable_package
                )
                ...
                if self.allow_cvss_v3_severity(
                    allowlist_format_vulnerability_object
                ) or self.allow_vendor_severity_override(
                    allowlist_format_vulnerability_object
                ):
                    continue
                if (
                ...
                self.vulnerability_list[
                    allowlist_format_vulnerability_object.package_name
                ].append(allowlist_format_vulnerability_object)
...
def get_target_image_uri_using_current_uri_and_target_repo(
    ...
    if append_tag:
        upgraded_image_tag = (
            f"{original_image_repository}-{original_image_tag}-{append_tag}"
        )
    else:
...
def _invoke_lambda(function_name, payload_dict={}):
    ...
    if status_code != 202:
        raise ValueError(
            "Lambda call not made properly. Status code returned {status_code}"
        )
...
def create_and_save_package_list_to_s3(
    old_filepath, new_packages, new_filepath, s3_bucket_name
):
    """
    ...
    s3_client.upload_file(
        Filename=new_filepath, Bucket=s3_bucket_name, Key=new_filepath
    )
...
def get_vulnerabilites_fixable_by_upgrade(
    ...
    fixable_allowlist_vulnerabilites = (
        image_allowlist - upgraded_image_vulnerability_list
    )
...
def conduct_failure_routine(
    ...
    newly_found_non_fixable_vulnerabilites = (
        upgraded_image_vulnerability_list - image_allowlist
    )
    fixable_list = {}
    ...
    new_package_list = (
        fixable_list if isinstance(fixable_list, list) else list(fixable_list.keys())
    )
    ...
    if newly_found_non_fixable_vulnerabilites:
        newly_found_non_fixable_list = (
            newly_found_non_fixable_vulnerabilites.vulnerability_list
        )
    message_body = {
    ...
    if (
        test_utils.is_canary_context()
        and os.getenv("REGION") == test_utils.DEFAULT_REGION
    ):
...
def process_failure_routine_summary_and_store_data_in_s3(
    ...
    save_details.append(
        (
            ...
            failure_routine_summary["fixable_vulnerabilities"],
        )
    )
    save_details.append(
        (
            s3_filename_for_non_fixable_list,
            failure_routine_summary["non_fixable_vulnerabilities"],
        )
    )
    ...
    while (time() - start_time) <= 45 * 60:
        try:
            (
                scan_status,
                scan_status_description,
            ) = ecr_utils.get_ecr_image_enhanced_scan_status(ecr_client, image)
...
def fetch_other_vulnerability_lists(image, ecr_client, minimum_sev_threshold):
    ...
    """
    new_image_uri_for_upgraded_image = (
    ...
    scan_results_with_upgrade = ecr_utils.get_ecr_image_scan_results(
        ecr_client,
        new_image_uri_for_upgraded_image,
        minimum_vulnerability=minimum_sev_threshold,
    )
