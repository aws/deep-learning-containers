...
def host_setup_for_tensorflow_inference(container_name, framework_version):
    ...
    context.run(f"python3 -m virtualenv {venv_location}", echo=True)
    ...
    context.run(f"source {venv_location}/bin/activate")
    ...
    context.run("deactivate")
    return src_location, venv_location, model_path
