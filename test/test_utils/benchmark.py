...
def execute_single_node_benchmark(context, image_uri, framework, task, py_version, script_url):
    ...
    container_name = (
        f"{framework}_single_node-{image_uri.split('/')[-1].replace('.', '-').replace(':', '-')}"
    )
    ...
    execute_cmd_on_container(
        context,
        container_name,
        f"curl http://{benchmark_endpoint}/api/tools/{benchmark_exec} " f"-o {benchmark_exec}",
    )
    ...
    execute_cmd_on_container(
        context, container_name, f"./{benchmark_exec} --set-client-id {benchmark_client_id}"
    )
...
def execute_cmd_on_container(context, container_name, bash_command, warn=False):
    ...
    """
    return context.run(
        f"docker exec --user root {container_name} bash -c '{bash_command}'", hide=True, warn=warn
    )
