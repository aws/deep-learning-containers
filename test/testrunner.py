...
def run_sagemaker_pytest_cmd(image):
    ...
    path = os.path.join("sagemaker", framework, job_type)
    ...
    try:
        ...
        subprocess.run(f"source {framework}_{job_type}/bin/activate", shell=True)
        subprocess.run(f"pip install -r {path}/requirements.txt", shell=True)

    except subprocess.CalledProcessError as e:
        raise EnvironmentError("Can't acitvate the virtual env for running pytest commands")
    # pytest.main(cmd)
