...
"
  ...
  # Truncate release notes to first 50 lines
  local release_notes_summary=""
  if [[ -n "${release_body}" ]]; then
    release_notes_summary=$(echo "${release_body}" | head -50)
    local line_count
    line_count=$(echo "${release_body}" | wc -l)
    if [[ ${line_count} -gt 50 ]]; then
      release_notes_summary="${release_notes_summary}

... [truncated - see full release notes](${html_url})"
    fi
  else
...
for framework in ${FRAMEWORKS}; do
  ...
  (
    ...
    release_body=$(echo "${api_body}" | jq -r '.body // ""')
